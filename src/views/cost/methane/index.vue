<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="沼气表信息" width="300px" align="center">
        <template slot-scope="scope">
          <span style="float:left;"><span style="font-weight:bold;">编号/名称：</span>{{ scope.row.methaneNumber }}&#12288;&#12288;{{ scope.row.methaneName }}</span><br>
      ...
      <el-table-column label="使用量(m³)" width="190px" align="center">
        <template slot-scope="scope">
          ...
          <span style="float:left;"><span style="font-weight:bold;">当天用气量</span>{{ scope.row.methaneConsumption }}</span><br>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        <el-row>
          <el-col :span="8">
            <el-form-item label="沼气表编号：" prop="formNumber">
              <el-autocomplete
                ref="formNumber"
          ...
          <el-col :span="8">
            <el-form-item label="沼气表名称：" prop="formName">
              <el-autocomplete
                ref="formName"
...
export default {
  ...
  data() {
    return {
      ...
      // 2-1.请求下拉框接口
      requestParams: [
        { name: 'getDictByName', offset: 0, pagecount: 0, params: ['ZZ流量计类型'] },
  ...
  methods: {
    ...
    formNumberSearch(queryString, cb) {
      ...
      this.requestFilterParams.parammaps['formType'] = 'ZZ流量计'
    ...
    handleformNumberSelect() {
      this.requestFilterParams.name = 'findByNMeasureMethane'
    ...
    createData() {
      this.$refs['temp'].validate(valid => {
        if (valid) {
          this.requestParam.name = 'insertMethane'
    ...
    updateData() {
      this.$refs['temp'].validate(valid => {
        if (valid) {
          this.requestParam.name = 'updatMethane'
          this.requestParam.parammaps = this.temp
          PostDataByName(this.requestParam).then(response => {
            console.log(response)
            if (response.msg === 'fail') {
    ...
    handleDelete(row) {
      MessageBox.confirm('设备名称：' + row.equipmentName, '确认删除？', {
        ...
        this.requestParam.name = 'deleteMethane'
